#!/usr/bin/env python3
"""Execute the frozen Trace the Ace M1-prime estimator-repair successor.

M1-prime preserves the M1 representation and changes only the iteration ceiling.
"""
...
def paired_session_bootstrap(
    ...
    parent_loss: np.ndarray,
    replicates: int,
    seed: int,
) -> dict[str, float]:
    tmp = pd.DataFrame({"session_id": session_ids, "delta": candidate_loss - parent_loss})
    ...
    return {
        "point_delta_log_loss": float((candidate_loss - parent_loss).mean()),
...
def make_vectorizer(cfg: dict) -> HashingVectorizer:
    dtype = np.float32 if str(cfg["dtype"]) == "float32" else np.float64
...
def main() -> None:
    ...
    ap.add_argument("--m1-oof", required=True, type=Path)
    ...
    if cfg["experiment_id"] != "M1_PRIME":
        raise AssertionError("wrong experiment config")
    if bool(cfg["objective_information_allowed"]):
        raise AssertionError("M1-prime objective information must be disabled")
    if bool(cfg["cca_derived_features_allowed"]):
        raise AssertionError("M1-prime CCA-derived features must be disabled")
    ...
    m1_oof = pd.read_csv(args.m1_oof)
    ...
    if not m1_oof["response_id"].is_unique:
        raise AssertionError("M1 OOF response_id must be unique")
    ...
    parent_col = "m1_probability" if "m1_probability" in m1_oof.columns else "probability"
    df = df.merge(
        m1_oof[["response_id", parent_col]].rename(columns={parent_col: "m1_probability"}),
        on="response_id",
        validate="one_to_one",
    )
    required_join = ["response_id", "session_id", "is_correct", "fold", "m0_probability", "m1_probability"]
    if df[required_join].isna().any().any():
        raise AssertionError("required M1-prime join fields contain missing values")
    ...
    X_session = build_session_text_matrix(sessions, path_map, vectorizer, role_markers, args.batch_size)
    ...
    for fold in expected_folds:
        ...
        Xtr = sparse.hstack([X_text[tr_idx], sparse.csr_matrix(tr_struct)], format="csr", dtype=np.float32)
        Xva = sparse.hstack([X_text[va_idx], sparse.csr_matrix(va_struct)], format="csr", dtype=np.float32)
        ...
        parent_fold = df.iloc[va_idx]["m1_probability"].to_numpy(dtype=float)
        candidate_ll = float(log_loss(y[va_idx], pred))
        m0_ll = float(log_loss(y[va_idx], m0_fold))
        parent_ll = float(log_loss(y[va_idx], parent_fold))
        fold_records.append(
            {
                "fold": int(fold),
                ...
                "m0_log_loss": m0_ll,
                "m1_log_loss": parent_ll,
                "m1_prime_log_loss": candidate_ll,
                "delta_log_loss_m1_prime_minus_m1": candidate_ll - parent_ll,
                "m1_prime_auc": float(roc_auc_score(y[va_idx], pred)),
    ...
    if not np.isfinite(oof).all():
        raise AssertionError("M1-prime OOF predictions incomplete")

    m0_p = df["m0_probability"].to_numpy(dtype=float)
    parent_p = df["m1_probability"].to_numpy(dtype=float)
    m0_ll = float(log_loss(y, m0_p))
    parent_ll = float(log_loss(y, parent_p))
    candidate_ll = float(log_loss(y, oof))
    candidate_auc = float(roc_auc_score(y, oof))
    calibration = calibration_summary(y, oof)
    parent_calibration = calibration_summary(y, parent_p)
    uncertainty = paired_session_bootstrap(
        df["session_id"].to_numpy(),
        per_row_log_loss(y, oof),
        per_row_log_loss(y, parent_p),
        int(cfg["uncertainty"]["replicates"]),
        int(cfg["uncertainty"]["random_seed"]),
    )

    bias = abs(calibration["mean_probability"] - calibration["observed_rate"])
    parent_bias = abs(parent_calibration["mean_probability"] - parent_calibration["observed_rate"])
    required_fraction = 1.0 - float(cfg["calibration"]["material_reduction_fraction"])
    max_iter = int(cfg["classifier"]["max_iter"])
    convergence_pass = all(int(r["n_iter"]) < max_iter for r in fold_records)
    objective_ranges = pd.DataFrame({"session_id": df["session_id"], "p": oof}).groupby("session_id")["p"].agg(lambda x: float(x.max() - x.min()))
    objective_exclusion_pass = bool(float(objective_ranges.max()) == 0.0)
    calibration_gate = bool(
        calibration["ece_10_equal_width"] <= required_fraction * parent_calibration["ece_10_equal_width"]
        and bias <= required_fraction * parent_bias
        and calibration["brier_score"] <= parent_calibration["brier_score"]
        and candidate_ll <= parent_ll
    )
    baseline_pass = bool(convergence_pass and calibration_gate and objective_exclusion_pass)
    ...
    pd.DataFrame(
        {
            ...
            "is_correct": y,
            "m0_probability": m0_p,
            "m1_probability": parent_p,
            "m1_prime_probability": oof,
        }
    ).sort_values("response_id").to_csv(args.output_dir / "oof_predictions.csv", index=False, lineterminator="\n")
    ...
    result = {
        "schema_version": 1,
        "experiment_id": "M1_PRIME",
        "parent_experiment_id": "M1",
        "status": "DIAGNOSED_PASS" if baseline_pass else "DIAGNOSED_UNRESOLVED",
        "objective_information_present": False,
        "cca_derived_features_present": False,
        "data": {"responses": int(len(df)), "sessions": int(df["session_id"].nunique())},
        "identity": {
            ...
            "m1_oof_sha256": sha256_file(args.m1_oof),
        ...
        "comparison": {
            ...
            "m1_pooled_log_loss_recomputed": parent_ll,
            "m1_prime_pooled_log_loss": candidate_ll,
            "delta_log_loss_m1_prime_minus_m1": candidate_ll - parent_ll,
            "m1_prime_pooled_auc": candidate_auc,
            "m1_calibration_recomputed": parent_calibration,
            "m1_prime_calibration": calibration,
            "m1_prime_absolute_mean_probability_bias": bias,
            "m1_absolute_mean_probability_bias_recomputed": parent_bias,
            "calibration_material_reduction_fraction": float(cfg["calibration"]["material_reduction_fraction"]),
        ...
        "diagnosis": {
            ...
            "objective_exclusion_pass": objective_exclusion_pass,
            "max_within_session_prediction_range": float(objective_ranges.max()),
            "baseline_gate_pass": baseline_pass,
            "m2_authorized": baseline_pass,
            "next_boundary_if_unresolved": None if baseline_pass else ("calibration_only_successor" if convergence_pass else "estimator_successor"),
        },
        "text_matrix": {"response_rows": int(X_text.shape[0]), "features": int(X_text.shape[1]), "nnz": int(X_text.nnz)},
        "environment": env,
        "authority": {
            "gained": (["M1_ordinary_semantic_baseline_closed_for_M2_comparison"] if baseline_pass else []),
            "inherited": ["generic_transcript_semantics_add_predictive_information_beyond_M0"],
            "not_gained": [
                "CCA_support",
                "CCA_refutation",
                "causal_evidence",
    ...
    (args.output_dir / "m1_prime_record.json").write_text(
