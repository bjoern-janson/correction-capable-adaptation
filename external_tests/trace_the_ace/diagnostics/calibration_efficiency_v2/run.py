...
def population_ece(session_sizes,p,q,bins=10):
    n=np.asarray(session_sizes,float); p=np.asarray(p,float); q=np.asarray(q,float); b=bin_index(p,bins); N=float(n.sum()); vals=[]
    for k in range(bins):
        m=b==k; vals.append(0.0 if not np.any(m) else abs(float(np.sum(n[m]*(p[m]-q[m]))/N)))
    return float(np.sum(vals))

def session_residual_aggregates(session_codes,p,y,n_sessions,bins=10):
    p=np.asarray(p,float); y=np.asarray(y,float); s=np.asarray(session_codes,int); b=bin_index(p,bins); key=s*bins+b; size=n_sessions*bins
    cnt=np.bincount(key,minlength=size).reshape(n_sessions,bins).astype(float); resid=np.bincount(key,weights=p-y,minlength=size).reshape(n_sessions,bins).astype(float); n_j=np.bincount(s,minlength=n_sessions).astype(float); return cnt,resid,n_j
...
def model_if(cnt,resid,n_j,regularity_threshold):
    ...
    regular=bool(np.all(z[nonempty]>=regularity_threshold)); if_total=(if_bin*signs[None,:]).sum(axis=1); theta=float(np.abs(mu).sum()); return {'theta':theta,'mu':mu,'nonempty':nonempty,'se_bin':se_bin,'z_bin':z,'regular':regular,'if_total':if_total}
...
def constitute_fixture(session_sizes,cfg):
    fx=cfg['synthetic_fixture']; S=len(session_sizes); u=(np.arange(S,dtype=float)+0.5)/S; pref=0.15+0.70*u; rb=bin_index(pref,10); mag=float(fx['reference_bin_residual_magnitude']); sign=np.where(rb%2==0,1.0,-1.0); residual=mag*sign; q=pref-residual
    if np.any(q<=0) or np.any(q>=1): raise AssertionError('synthetic q outside unit interval')
    ref_ece=population_ece(session_sizes,pref,q); lams=np.linspace(float(fx['lambda_grid_min']),float(fx['lambda_grid_max']),int(fx['lambda_grid_points'])); targets={k:float(v) for k,v in fx['scenarios'].items()}; chosen={}; vals=[]
    for lam in lams:
        pc=pref+lam; vals.append(population_ece(session_sizes,pc,q)-ref_ece)
    vals=np.asarray(vals,float); tol=float(fx['target_match_absolute_tolerance'])
    for name,target in targets.items():
        j=int(np.argmin(np.abs(vals-target))); err=abs(float(vals[j]-target))
        if err>tol: raise AssertionError(f'target not reachable {name}: err={err}')
        pc=pref+lams[j]; change=float(np.mean(bin_index(pc)!=rb)); chosen[name]={'target_delta':target,'lambda':float(lams[j]),'true_delta':float(vals[j]),'target_abs_error':err,'bin_membership_change_fraction':change}
    return pref,q,ref_ece,chosen

def synthetic_regular(session_sizes,cfg):
    fx=cfg['synthetic_fixture']; pref,q,ref_ece,chosen=constitute_fixture(session_sizes,cfg); zcrit=float(cfg['uncertainty_candidate']['z_critical']); rz=float(cfg['uncertainty_candidate']['regularity_gate_z']); start=int(fx['outcome_seeds_start']); count=int(fx['outcome_seed_count']); records=[]
    for seed in range(start,start+count):
        ...
        for name,spec in chosen.items():
            pc=pref+float(spec['lambda']); cc,rc,nc=constant_session_prediction_aggregates(session_sizes,pc,ysum); rec=paired_if_interval(cr,rr,nr,cc,rc,nc,zcrit,rz); true=float(spec['true_delta']); rec.update({'dataset_seed':seed,'scenario':name,'true_delta_ece':true,'covers_true_delta':bool(rec['ci95_lower']<=true<=rec['ci95_upper']),'passes_noninferiority':bool(rec['ci95_upper']<float(cfg['scientific_target']['epsilon_ece']))}); records.append(rec)
    summary={}
    for name in chosen:
