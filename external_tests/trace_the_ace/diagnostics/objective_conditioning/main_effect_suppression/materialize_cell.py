...
def main():
 ...
 o=a.outer_fold;tr=np.flatnonzero(fa!=o);va=np.flatnonzero(fa==o);assert not(set(g[tr])&set(g[va]));qs,qv=struct(Xq[tr],Xq[va]);cr,cv=center(R,obj,tr,va);base0tr=[Xt[tr],sparse.csr_matrix(qs),sparse.csr_matrix(Xs[tr])];base0va=[Xt[va],sparse.csr_matrix(qv),sparse.csr_matrix(Xs[va])];base1tr=[Xt[tr],Xo[tr],sparse.csr_matrix(qs),sparse.csr_matrix(Xs[tr])];base1va=[Xt[va],Xo[va],sparse.csr_matrix(qv),sparse.csr_matrix(Xs[va])];spec={'B0':(base0tr,base0va),'C0':(base0tr+[sparse.csr_matrix(cr)],base0va+[sparse.csr_matrix(cv)]),'B1':(base1tr,base1va),'C1':(base1tr+[sparse.csr_matrix(cr)],base1va+[sparse.csr_matrix(cv)])};tb,vb=spec[a.arm];X=sparse.hstack(tb,format='csr',dtype=np.float32);V=sparse.hstack(vb,format='csr',dtype=np.float32)
 out=a.output_dir;out.mkdir(parents=True,exist_ok=True);prefix=f'{a.arm}_fold{o}';sparse.save_npz(out/f'{prefix}_X.npz',X,compressed=False);sparse.save_npz(out/f'{prefix}_V.npz',V,compressed=False);np.savez(out/f'{prefix}_meta.npz',y_train=y[tr].astype(np.int8),row_index=va.astype(np.int32));rec={'arm':a.arm,'fold':o,'X_shape':X.shape,'V_shape':V.shape,'X_nnz':int(X.nnz),'V_nnz':int(V.nnz),'X_data_sha256':hashlib.sha256(X.data.tobytes()).hexdigest(),'X_indices_sha256':hashlib.sha256(X.indices.tobytes()).hexdigest(),'X_indptr_sha256':hashlib.sha256(X.indptr.tobytes()).hexdigest(),'V_data_sha256':hashlib.sha256(V.data.tobytes()).hexdigest(),'V_indices_sha256':hashlib.sha256(V.indices.tobytes()).hexdigest(),'V_indptr_sha256':hashlib.sha256(V.indptr.tobytes()).hexdigest()};(out/f'{prefix}_matrix_record.json').write_text(json.dumps(rec,sort_keys=True)+'\n');print(json.dumps(rec),flush=True);sys.stdout.flush();sys.stderr.flush();os._exit(0)
