...
def center(R,obj,tr,va):
 ...
 return a,b
def fit(parent,cfg,trb,vab,y):
 X=sparse.hstack(trb,format='csr',dtype=np.float32);V=sparse.hstack(vab,format='csr',dtype=np.float32);m=parent.make_classifier(cfg);m.fit(X,y);return sigmoid(np.asarray(m.decision_function(V),float)),int(np.asarray(m.n_iter_).max())
def main():
 p=argparse.ArgumentParser();p.add_argument('--outer-fold',type=int,required=True);p.add_argument('--config',type=Path,required=True);p.add_argument('--index',type=Path,required=True);p.add_argument('--folds',type=Path,required=True);p.add_argument('--m0-session-features',type=Path,required=True);p.add_argument('--zt-cache',type=Path,required=True);p.add_argument('--session-semantic',type=Path,required=True);p.add_argument('--response-conditioning',type=Path,required=True);p.add_argument('--eligible-objectives',type=Path,required=True);p.add_argument('--parent-runner',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();cfg=yaml.safe_load(a.config.read_text());parent=loadmod('parent',a.parent_runner)
 ...
 o=a.outer_fold;tr=np.flatnonzero(fa!=o);va=np.flatnonzero(fa==o);assert not(set(g[tr])&set(g[va]));qs,qv=struct(Xq[tr],Xq[va]);cr,cv=center(R,obj,tr,va);b0tr=[Xt[tr],sparse.csr_matrix(qs),sparse.csr_matrix(Xs[tr])];b0va=[Xt[va],sparse.csr_matrix(qv),sparse.csr_matrix(Xs[va])];b1tr=[Xt[tr],Xo[tr],sparse.csr_matrix(qs),sparse.csr_matrix(Xs[tr])];b1va=[Xt[va],Xo[va],sparse.csr_matrix(qv),sparse.csr_matrix(Xs[va])];spec={'B0':(b0tr,b0va),'C0':(b0tr+[sparse.csr_matrix(cr)],b0va+[sparse.csr_matrix(cv)]),'B1':(b1tr,b1va),'C1':(b1tr+[sparse.csr_matrix(cr)],b1va+[sparse.csr_matrix(cv)])};out={'row_index':va.astype(np.int32)}
 for arm,(tb,vb) in spec.items():
  pr,ni=fit(parent,cfg['base_classifier'],tb,vb,y[tr]);out[f'{arm}_p']=pr.astype(np.float64);out[f'{arm}_n_iter']=np.asarray([ni],np.int32);print(json.dumps({'fold':o,'arm':arm,'n_iter':ni,'rows':len(va)}),flush=True)
 a.output.parent.mkdir(parents=True,exist_ok=True);np.savez_compressed(a.output,**out)
