...
def main():
 p=argparse.ArgumentParser();p.add_argument('--config',type=Path,required=True);p.add_argument('--index',type=Path,required=True);p.add_argument('--folds',type=Path,required=True);p.add_argument('--eligible-objectives',type=Path,required=True);p.add_argument('--fold-dir',type=Path,required=True);p.add_argument('--output-dir',type=Path,required=True);a=p.parse_args();cfg=yaml.safe_load(a.config.read_text());idx=pd.read_csv(a.index,usecols=['response_id','session_id','learning_objective_id','is_correct']);fd=pd.read_csv(a.folds);df=idx.merge(fd,on='session_id',validate='many_to_one');elig=set(a.eligible_objectives.read_text().splitlines());df=df[df.learning_objective_id.astype(str).isin(elig)].reset_index(drop=True);y=df.is_correct.to_numpy(np.int8);g=df.session_id.astype(str).to_numpy();arms=['B0','C0','B1','C1'];pred={k:np.full(len(df),np.nan) for k in arms};iters={k:[] for k in arms};seen=set()
 for f in range(5):
  z=np.load(a.fold_dir/f'fold{f}.npz');ix=z['row_index'].astype(int);assert not seen.intersection(map(int,ix));seen.update(map(int,ix))
  for k in arms:pred[k][ix]=z[f'{k}_p'];iters[k].append(int(z[f'{k}_n_iter'][0]))
 if seen!=set(range(len(df))) or any(not np.isfinite(pred[k]).all() for k in arms):raise AssertionError('coverage')
